...
mod tests {
    ...
    #[test]
    fn daily_digest_same_day_after_hour() {
        let anchor = at(18, 9);
        let now = at(18, 10);
        let next = next_digest_after(anchor, 9, 1, now).unwrap();
        assert_eq!(next, at(18, 9));
    }
    ...
    fn three_day_digest_steps_correctly() {
        let anchor = at(16, 9);
        let now = at(19, 8);
        let next = next_digest_after(anchor, 9, 3, now).unwrap();
...
pub fn next_digest_after(
    ...
    let hour = digest_hour.clamp(0, 23) as u32;
    let mut candidate = anchor;
    // Move to the digest hour on the anchor day in Vienna time
    candidate = candidate
        .with_timezone(&Vienna)
        .date_naive()
        .and_time(NaiveTime::from_hms_opt(hour, 0, 0)?)?
    ...
    while candidate <= now {
        candidate = candidate + Duration::days(frequency_days.max(1) as i64);
    }
    Some(candidate)
}
```

Note: the spec says due when `next_digest <= now`. Adjust the helper semantics so callers compare `<= now`. The implementation above returns the first digest strictly after `now`; either adjust or document that callers use the previous candidate. Simpler: return the first digest on/after `anchor` and let the caller decide if it is `<= now`.

Refine to:

```rust
pub fn digest_moments(anchor: DateTime<Utc>, digest_hour: i32, frequency_days: i32) -> impl Iterator<Item = DateTime<Utc>> { ... }
```

Then callers take the first moment `> batch.created_at` and check if `<= now`.
